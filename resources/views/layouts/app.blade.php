...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'Appointifi') }}</title>
    ...
    <body class="font-sans antialiased bg-gray-50 dark:bg-gray-900" x-data="{ darkMode: localStorage.getItem('darkMode') === 'true' || @json(session('dark_mode', false)) }" x-init="$watch('darkMode', value => localStorage.setItem('darkMode', value))">
        <div class="min-h-screen flex">
            ...
            <div class="flex-1 flex flex-col overflow-hidden">
                ...
                <header class="md:hidden bg-white dark:bg-gray-800 shadow-sm sticky top-0 z-40" x-data>
                    <div class="flex items-center justify-between p-4">
                        <h1 class="text-xl font-semibold text-gray-900 dark:text-white">{{ config('app.name', 'Appointifi') }}</h1>
