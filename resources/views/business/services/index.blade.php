                    ...
                    @foreach ($services as $service)
                        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 p-6">
                            ...
                            <div class="flex gap-2 pt-4 border-t border-gray-200 dark:border-gray-700">
                                <a href="{{ route('business.services.edit', $service) }}" class="flex-1 px-4 py-2 bg-white dark:bg-gray-700 border rounded-lg text-sm">Edit</a>
                                <form method="POST" action="{{ route('business.services.destroy', $service) }}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="px-4 py-2 bg-red-50 dark:bg-red-900/20 border rounded-lg text-sm text-red-700 dark:text-red-400">Delete</button>
                                </form>
                            </div>
                        </div>
                    @endforeach
